    ...
    server.route([
        ...
        },
        {
            ...
            config: {
                ...
                handler: function (request, reply) {
                    return reply.file(path.join('public/img', request.params.filename)).header('cache-control', 'max-age=60');
                },
                validate: {
                    params: {
        ...
        },
        {
            ...
            config: {
                ...
                handler: function (request, reply) {
                    return reply.file(path.join('public/ttf', request.params.filename)).header('cache-control', maxAge);
                },
                validate: {
                    params: {
                        filename: joi.string().regex(/^.*\.(ttf|woff)$/)
                    }
                },
                tags: ['assets']
            }
        ...
        },
        {
            ...
            path: '/{param*}',
            handler: function (request, reply) {
                return reply(request.server.app.renderView('dashboard'));
            },
            config: {
                tags: ['web']
            }
