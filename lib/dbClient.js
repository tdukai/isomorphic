...
local.aggregateCounts = function aggregateCounts (db) {
...
class DbClient {
    ...
    *
    * @method lookup
    * @param {string} field (country, region, city, postalCode)
    * @param {string} search phrase (first couple of letters to match on)
    * @param {number} maximum number of results expected
    *
    * @return {object} Promise object
    */
    lookup (field, search, count) {
        return new Promise(function (resolve, reject) {
            mongodb.MongoClient.connect(url).then(function (db) {
                var query = {};
                settings = {}
                query[field] = { $rejex: /m/i };

                db.collection('people').find(query, )
            }).catch(reject);
        });
    }
    ...
    stats () {
        ...
        return new Promise(function (resolve, reject) {
            mongodb.MongoClient.connect(url).then(function (db) {
                ...
                result;
                // Add the tasks
                tasks.push(local.aggregateCounts(db));
                ...
                Promise.all(tasks).then(function (results) {
                    ...
                    result = results[0];
                    result.counts.divorced = results[1];
                    result.counts.region = results[2].length;
                    result.counts.city = results[3].length;
                    result.counts.country = results[4].length;
                    // Return
                    console.log(result); 
